...
export async function initializeDatabase(): Promise<void> {
	...
	try {
		db.exec("ALTER TABLE transcriptions ADD COLUMN source TEXT NOT NULL DEFAULT 'upload'");
...
export function saveTranscription(
	...
	const stmt = database.prepare(
		'INSERT INTO transcriptions (filename, text, language, confidence, source) VALUES (?, ?, ?, ?, ?)'
	);
	
	const result = stmt.run(filename, text, language, confidence, source);
...
export function getAllTranscriptions(limit = 100, offset = 0): TranscriptionRecord[] {
	...
	const stmt = database.prepare(
		'SELECT id, filename, text, language, confidence, source, created_at FROM transcriptions ORDER BY created_at DESC LIMIT ? OFFSET ?'
	);
